...
if [[ $# -gt 0 ]] ; then
    ...
    base_subjects=$( cd /data/sanDiego ; ls -d [0-9][0-9][0-9]_{B,C,D,E} [0-9][0-9][0-9]_{B,C,D,E}2 2> /dev/null | grep -v 999 )
    subjects=""
    for subject in $base_subjects ; do
...
echo "Trying to reconstruct the following $(echo $subjects | wc -w ) subjects:"
echo $subjects
